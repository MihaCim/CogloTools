...
class DictHandler(sax.handler.ContentHandler):
    ...
    def startElement(self, name, attrs):
        ...
        if name == "node":
            self.aktiv = "node"
            self.db.addPath(attrs["id"], node(attrs["id"], float(attrs["lat"]), float(attrs["lon"])))
        # searching for ways
        if name == "way":
...
class node():
    ...
    def discardChanges(self):
        print(
            "removing connection from nonway")
        self.tempconnections = []
    ...
    def printNextNodes(self):
        ...
        for i in self.connections:
            ...
            print(
                "id: " + otherNode.id + " lat: " + otherNode.lat + " lon: " + otherNode.lon + " distance: " + str(
    ...
    def raiseKnowledge(self, prefNode, destNode):
        ...
        print(
            "\t distanceFromPrefNode: \t" + str(distanceFromPrefNode))
        self.distanceFromStart = prefNode.distanceFromStart + distanceFromPrefNode
        print(
            "\t distanceFromStart: \t " + str(self.distanceFromStart))
        self.distanceToDest = calcDistance(self, destNode)
        print(
            "\t distanceToDest: \t" + str(self.distanceToDest))
...
class routingDb():
    ...
    def printStatus(self):
        print(
            "<------------ Printing Status --------------->")
        print(
            "We have: " + self.size() + " Nodes")
        for i in self.listOfNodes:
            print(
                "Node " + str(i) + " is in da DB it has: " + str(
...
def startRouting(db):
    ...
    print(
        "start Ziehl distanz: " + str(calcDistance(startNode, endNode)))
    ...
    print(
        "I came to " + nowNode.id)
    while (nowNode.cameFrom != None):
        print(
            " via " + nowNode.cameFrom.id + "\t Link: http://www.openstreetmap.org/node/" + nowNode.cameFrom.id)
        nowNode = nowNode.cameFrom


db = routingDb()
print()
readDb(open(filepath, "r"), db)
db.cleanUp()
startRouting(db)
